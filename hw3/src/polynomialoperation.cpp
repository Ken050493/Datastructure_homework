...
class Polynomial {
 private:
  ...
  int termCount() const {
    int cnt = 0;
    for (Node* p = head->link; p != head; p = p->link) ++cnt;
    return cnt;
  }
 ...
 public:
  ...
  Polynomial operator+(const Polynomial& b) const {
    ...
    while (pa != head && pb != b.head) {
      if (pa->exp == pb->exp) {
        ...
        Node* t = getNode(pa->coef, pa->exp);
        tail->link = t;
        tail = t;
        pa = pa->link;
      } else {
        Node* t = getNode(pb->coef, pb->exp);
        tail->link = t;
        tail = t;
        pb = pb->link;
      }
  ...
  friend ostream& operator<<(ostream& os, const Polynomial& x) {
    os << x.termCount();
    for (Node* p = x.head->link; p != x.head; p = p->link)
      os << ' ' << p->coef << ' ' << p->exp;
    return os;
  }
};
