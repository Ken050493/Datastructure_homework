...
1.用帶表頭的循環串列存多項式。節點含係數與指數。  
2.寫一個 insertTerm(c,e)。維持指數遞減。同指數相加。變 0 就刪。  
3.加減：兩串列同步走，比指數，大的先放，指數相同就相加。  
4.乘法：雙迴圈逐項相乘，結果一律丟給 insertTerm 合併。  
5.Evaluate：逐項累加 coef * x^exp。  
6.I/O：讀 n 組 (c,e) 逐項插入；輸出為 n c1 e1 ... cn en。  
